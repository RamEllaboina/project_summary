...
`;
    ...
    async runContainer(imageName, commands, projectInfo) {
        ...
        try {
            ...
            let containerPort = 0;
            if (projectInfo.type === 'static') containerPort = 3000;
            else if (projectInfo.type === 'nodejs') containerPort = 3000;
            else if (projectInfo.type === 'python') containerPort = 8000;
            else if (projectInfo.type === 'java' || projectInfo.type === 'go' || projectInfo.type === 'rust') containerPort = 8080;

            if (containerPort > 0) {
                // For anything with a likely server port, run in detached mode mapping the port
                const runCmd = `docker run -d --name ${imageName}-runtime -p 0:${containerPort} --memory=${this.memoryLimit} --cpus=${this.cpuLimit} ${imageName}`;

                // Start container
                await execPromise(runCmd, { timeout: this.executionTimeout });

                // Wait for container to fully start (might need more time for some frameworks)
                ...
                if (!statusOutput.includes('Up')) {
                    // Container exited. It might have been a simple script that finishes, or it crashed.
                    // Let's grab the logs
                    const { stdout: logs } = await execPromise(`docker logs ${imageName}-runtime`);
                    const executionTime = Date.now() - startTime;
                    return {
                        logs: logs.trim() || 'Script executed and finished successfully.',
                        executionTime,
                        success: true
                    };
                }
                ...
                // Get the mapped port
                const portCmd = `docker port ${imageName}-runtime ${containerPort}`;
                try {
                    ...
                    // Could not get port, fallback to simple logs
                    const { stdout: logs } = await execPromise(`docker logs ${imageName}-runtime`);
                    return {
                        logs: logs.trim(),
                ...
                // Fallback for completely unknown generic types without ports, run synchronously
